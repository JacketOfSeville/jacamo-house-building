...
+!create_auction_artifact(Task,MaxPrice)
   <- .concat("auction_for_",Task,ArtName); // Concatena a string com 'Task' e armazena em 'ArtName'
      makeArtifact(ArtName, "tools.EnglishAuctionArt", [Task, MaxPrice], ArtId); // cria o artefato com o nome, tarefa, preço e ID
...
+!show_winners
   <- for ( currentWinner(Ag)[artifact_id(ArtId)] ) {
         ?currentPrice(Price)[artifact_id(ArtId)]; // verifica o lance atual
